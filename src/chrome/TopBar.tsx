...
import { applyTheme, getInitialTheme, subscribeToSystemTheme, type Theme } from "../lib/theme";
...
export function TopBar() {
  ...
  useEffect(() => {
    ...
    setTheme(initial);
    applyTheme(initial);
    // Follow the OS for users who haven't explicitly toggled. Bedtime,
    // OS-wide dark-mode schedule, etc. all flip Forkly in sync.
  ...
  // App-wide channel for "show me the upstream call" — error rows in chat
  // bubbles fire this event so the user can jump straight from a failed
  // message to its underlying request without learning the shortcut first.
  useEffect(() => {
    const handler = () => setNetLogOpen(true);
    window.addEventListener("forkly:open-network-log", handler);
    return () => window.removeEventListener("forkly:open-network-log", handler);
  }, []);
  ...
  const handleAdd = (override?: {
    ...
    addSession({
      position: { x: cx + jitterX, y: cy + jitterY },
      providerId: override?.providerId,
      modelId: override?.modelId,
      transportId: override?.transportId,
    }).catch((err) => {
  ...
  return (
    <header className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-center justify-between px-4 py-3">
      ...
      <div className="pointer-events-auto flex items-center gap-2">
        {/* Group 1 — help & app-level utilities (least-frequent actions) */}
        <div className="flex items-center gap-1 rounded-lg border border-(--glass-border) bg-(--glass-bg) backdrop-saturate-140 p-0.5 backdrop-blur">
          <HelpPopover />
          <Tooltip label="auth & api keys">
            <SettingsDialog
              trigger={
                <button
                  aria-label="Settings"
                  className="flex h-7 w-7 items-center justify-center rounded-md text-fg-muted transition-colors hover:bg-bg hover:text-fg"
                >
        ...
        {/* Group 2 — canvas tools (per-workspace controls) */}
        <div className="flex items-center gap-1 rounded-lg border border-(--glass-border) bg-(--glass-bg) backdrop-saturate-140 p-0.5 backdrop-blur">
        ...
        {/* Group 3 — create (primary action cluster) */}
        <Tooltip label="import a claude code session">
          <CcImportDialog
            trigger={
              <button
                aria-label="Import Claude Code session"
                className="flex h-8 items-center gap-1.5 rounded-lg border border-(--glass-border) bg-(--glass-bg) backdrop-saturate-140 px-2.5 font-mono text-xs text-fg-muted backdrop-blur transition-colors hover:text-fg"
              >
        ...
        <Tooltip label="new session — pick an LLM">
          <ProviderPicker
            ...
            trigger={
              <button
                ...
                className="flex h-8 items-center gap-2 rounded-lg border border-accent-from/60 bg-accent-from/15 px-3 font-mono text-xs text-fg backdrop-blur transition-colors hover:border-accent-from hover:bg-accent-from/25"
              >
